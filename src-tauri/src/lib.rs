        ...
        if !title.is_empty() && title != "Program Manager" && title != "Settings" {
            ...
            if !exe_path.is_empty() && !exe_path.contains("oasis-shell") {
                ...
                let is_duplicate = windows
                    .iter()
                    .any(|w| w.exe_path == exe_path && w.title == title);
...
async fn semantic_search(
...

    {
        let conn = state.0.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT filename, filepath, vector FROM file_embeddings")
            .unwrap();
        let rows = stmt
            .query_map([], |row| {
                Ok((
                ...
                if let Some(screen) = screens.first() {
                    if let Ok(image) = screen.capture() {
                        ...
                        if let Some(b64) = encode_ok {
                            ...
                            let llava_ready = tags_res
                                .ok()
                                .and_then(|r| r.json::<serde_json::Value>().ok())
                                .and_then(|j| {
                                    j["models"].as_array().map(|models| {
                                        models.iter().any(|m| {
                                            m["name"].as_str().unwrap_or("").contains("llava")
                                        })
                                    })
                                })
                                .unwrap_or(false);
                            ...
                            if !llava_ready {
                                println!(
                                    "Photographic Memory: Waiting for llava model to download..."
                                );
                                continue;
                            }
                                ...
                                .send()
                            {
                                if let Ok(json) = res.json::<serde_json::Value>() {
                                    if let Some(desc) = json["response"].as_str() {
                                        ...
                                        println!(
                                            "LLM response missing 'response' field: {:?}",
                                            json
                                        );
                                    }
                                } else {
