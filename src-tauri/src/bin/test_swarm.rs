...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let res = client.post("http://localhost:11434/api/generate").json(&chat_body).send().await?;
    ...
    let mut log_output = format!("=== Swarm Agent Autonomous Test ===\nTimestamp: {:?}\n\nThought Trace:\n{}\n\n", SystemTime::now(), response_text);
    ...
    if let Some(start_idx) = response_text.find("[CMD]") {
        if let Some(end_idx) = response_text.find("[/CMD]") {
            if end_idx > start_idx + 5 {
                ...
                log_output.push_str(&format!("Terminal Result:\nSTDOUT:\n{}\nSTDERR:\n{}\n", stdout, stderr));
                success = true;
            }
        }
    } else {
